...
function EventSource(url, eventSourceInitDict) {
  ...
  function onConnectionClosed(eventState) {
    if (connectPending || readyState === EventSource.CLOSED) return;
    connectPending = true;
    readyState = EventSource.CONNECTING;
    _emit('error', new Event('error'));
	  console.error('[eventsource] '+ JSON.stringify(eventState || {}));
  ...
  if (eventSourceInitDict && eventSourceInitDict.headers && isPlainObject(eventSourceInitDict.headers) && eventSourceInitDict.headers['Last-Event-ID']) {
    ...
    delete eventSourceInitDict.headers['Last-Event-ID'];
	  console.log('[eventsource] LastEventId is: ' + lastEventId + ' for url: '+ url);
  }
  ...
  function connect() {
    ...
    lastEventId && console.log('[eventsource] ConnectFn. LastEventId is: ' + lastEventId + ' for url: '+ url);
    (!lastEventId) && console.log('[eventsource] ConnectFn. LastEventId is missing/falsy.');
  ...
  function parseEventStreamLine(buf, pos, fieldLength, lineLength) {
    if (lineLength === 0) {
      ...
      if (field === 'data') {
        ...
        lastEventId = value;
        console.log('[eventsource] Parsed stream and updated lastEventId to: '+ lastEventId);
